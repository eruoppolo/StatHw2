...
# Define a range of p values, where pv are a range from 0.01 to 0.99.
...
plot(p_values, likelihood, type = "l", col = "red", xlab = "pv", ylab = "Likelihood",
     main = "Likelihood Function")

#results:
##we have y=3, the prob is given by: P(Y=3)=(1−p)^2⋅p
#we need to take pv as the estimated parameter
##the likelihood function is the function l(θ) = f(y;θ) of θ
```

In this exercise the first success has been observed after three samples. So, we are in a condition like: $P(Y=k) = (1-p)^{k-1}(p) \rightarrow P(Y=3) = (1-p)^2(p)$, where $p \in [0;1]$ is the success parameter.

The likelihood function of a Geometric distribution, represented in the above plot, has the following form:

$$L(p | \vec{y}) = \prod_{i = 1}^{n} f_i(y) = \prod_{i = 1}^{n} (1-p)^{y_i-1}p = ((1-p)^{\sum_{i = 1}^{n} y_i - n})(p^n)$$

The $MLE(p) = \frac{1}{\overline{y}}$.
...
Since, it is asked to estimate the proportion of students who believe in life after death, we assume students undecided as students who do not believe.
...
The number of observations in the dataset is 60.
...
The MLE estimate is 0.516.

The Wald confidence interval, at 95% of significance level is: $\hat\pi \pm z_{\frac{\alpha=0.05}{2}}\sqrt{\frac{\hat\pi(1-\hat\pi)}{n}}$.
...
The Wald confidence interval is $\hat\pi \in [0.3902 ; 0.6431]$.
...
Considering a significance level of $5\%$, the p-value of this test is much greater than 0.05, so the data does provide enough evidence to accept $H_0$, so there is evidence that the prices are distributed as a LogNormal distribution, with $\mu_{log} = 5.2913$ and $\sigma_{log} = 0.5591$.
...
Let $u=1200$ the number of Floridians which answered the poll, $n_1=624$ the number of people who voted for raising the taxes, which corresponds to the $\pi = 52\%$ and $n_2=576$ those who voted for reducing the services, $1-\pi = 48\%$.

The two hypothesis are: 

$$ \begin{cases}H_0 : \pi=0.50 \\
H_\alpha:\pi\neq 0.50
\end{cases} $$
...
Since $n$ is a large number we can use the normal distribution, and we use the two tailed test because we don't know which direction the distribution takes.
...
prop.test(n1, n, p=0.50, alt = "two.sided", conf.level = 0.95, correct = F)
```


We have also done the hypothesis testing using formulas and probability tables rather than using R libraries. So:
$$ z=\frac{\hat{p}-r_0}{\sqrt{\frac{r_0(1-r_0)}{n}}}$$

Where $\hat{p}$ is the proportion: $624/1200=0.52$. Hence we can compute this quantity:
$$ z=\frac{0.52-0.50}{\sqrt{\frac{0.50(1-0.50)}{1200}}}=1.4184 $$

From the normal quantile tables we can get the p-value:
$$ p = F(z<-1,4184)+F(z>-1,4184)=0.0778+0.0778=0.1556$$


P-value for testing $H_0: \pi = 0.50$ against $H_{\alpha}: \pi \neq 0.50$ is the two-tail probability of a test statistic at least as extreme as the observed $z = 1.4184$, which is $2(0.0778) = 0.1556$.

The difference in the two p-values is due to numerical approximation done by R.

We can conclude that the p-value obtained is greater than 0.1, which corresponds to a significance level of $10 \%$, which means that there are no marginal evidence to reject the $H_0$.
